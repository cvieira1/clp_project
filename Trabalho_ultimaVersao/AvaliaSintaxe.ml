...
let explode s =
	let rec exp i l =
		if i<0 then l else exp (i - 1) ((String.make 1 (String.get s i)) :: l) in
...
(* Função que verifica se a string esta em uma lista de string*)		
...
(* Função que retorna o primeiro indice em uma lista de strings *)
...
(* Função que verifica se a quantidade e ordem dos parenteses*)
(*  esta correta *)  
...
(* Função que verifica se a string eh true ou false *)
let verifica_logico = fun s resposta_valLogico ->
	  resposta_valLogico :=
...
(* Função que verifica se eh uma cadeia de caracteres *)
let verifica_cadeia = fun s resposta_cadeia ->
	 	let lista_string = explode s in
		let tamanho_string = List.length lista_string in
		let condicao = ref 0 in
		for i=0 to (tamanho_string - 1) do
			if (List.nth lista_string i) = "\"" then
				condicao := !condicao + 1;
		done;
		resposta_cadeia :=
		if !condicao == 2 then
