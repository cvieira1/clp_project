...
(* let verifica_sintaxeFun s lista listaAritmetico =                                                                                      *)
...
(* 					var := !var                                                                                                                  *)
(* 				(* controle igual a 1 significa que a variável anterior é número *)                                                            *)
(* 				else                                                                                                                           *)
(* 					if e_digito (String.get s i) && !controle = 1 then begin                                                                     *)
(* 						var := (!var)^(String.make 1 (String.get s i))                                                                             *)
(* 			 		end                                                                                                                         *)
(* 					else if e_digito (String.get s i) && !controle = 1                                                                           *)
(* 				else if (e_variavel_min (String.get s i) || e_variavel_max (String.get s i)) then begin                                        *)
...
let token = fun lista listaAritmetico listaLogico listaPalavraChave listaComparativo lista_final ->
  ...
  let oc2 = open_out arquivo2 in  
	...
	for i = 0 to (tamanho - 1) do
		...
		if i > 0 then
			begin
		(* Se for um operador aritmetico se torna uma aplicação aritmetica*)
      ...
      if !resposta_comparativo = 's' || !resposta_logico = 's' then
			  begin
					if (List.nth !lista i) = "<" then
						begin
						lista := replace !lista i "meno";
						end;
					if (List.nth !lista i) = ">" then
						begin
						lista := replace !lista i "maio";
						end;
				  lista_final := !lista_final@[("<aplBooleana,"^(List.nth !lista i)^","^(List.nth !lista (i-1))^","^(List.nth !lista (i+1))^">")];
					if (List.nth !lista i) = "meno" then
						begin
						lista := replace !lista i "<";
						end;
					if (List.nth !lista i) = "maio" then
						begin
						lista := replace !lista i ">";
						end;
			  end;
		    ...
		    if not (!resposta_valLogico) then
		      ...
		      if (ehVariavel (List.nth !lista i) 0 0 true) then
			      begin		
				    if i < (tamanho - 1) then
					    begin
		          (* Se o proximo for ( eh uma função *)
	  ...
	  if i < (tamanho - 1) then
   		...
   		if (List.nth !lista (i+1)) = "(" then
				begin	
				(* Verifica se não eh esta nos parametros de variavel *)	
		...
		if i > 0 then
		  ...
		  if contains	(List.nth !lista_final (i-1)) "sep" && (List.nth !lista i) = "," then
		    ...
		    Printf.fprintf oc2 "Error de sintaxe: Separador em posicao invalida";
			  exit 0;
			  end;
			if (contains (List.nth !lista_final (i-1)) "fun") && ((List.nth !lista i) = "(") && (List.nth !lista (i+1)) = ")" then
				...
				Printf.fprintf oc2 "Erro de sintaxe: Argumentos nao fornecidos";
				exit 0;
				end;	
		  end;
		if (List.nth !lista i) = "," then
		...
		if i < (tamanho - 1) then
   	  ...
   	  if (List.nth !lista (i+1)) = ":=" || ((List.nth !lista (i+1)) = ",") || ((List.nth !lista (i+1)) = ")") then
		    ...
		    if not (ehVariavel (List.nth !lista i) 0 0 true) && not !resposta_int && not !resposta_cadeia && not !resposta_valLogico && not !resposta_rac then
					...
					Printf.fprintf oc2 "Error de sintaxe: Nome de variavel com caracteres invalidos";
					exit 0;
					end;
				end;	
			end;	
		...
		if contains (List.nth !lista i) ":" && (List.nth !lista i) <> ":=" then		
		  ...
		  if !resposta_rac then	
			  ...
			  Printf.fprintf oc2 "Error de sintaxe: Racional em formato incorreto";		
			  exit 0;
			  end;
		  end;	
		resposta_rac := false;		
...
(* Funçao para remover elemento de certo indice de uma lista *)
	  ...
	  let resposta_rac_fator2 = ref false in
		...
		if !resposta_cadeia_fator1 || !resposta_cadeia_fator2 then
			begin
				...
				Printf.fprintf oc "Error: Operador nao compativel com tipo de dado!";
				exit 0;
				end;		
			end
		else
...
(* Funçao que executa aplicao a partir de um operador logico *)
...
let rec executaAplicacoes = fun lista_final listaVariaveis listaValores listaTipos var_atribuida->
  ...
  let aplica = ref false in
	...
	let ind_primeiroAtr = ref (0-1) in
  ...
  for i = 0 to (tamanho_lista_final - 1) do
      ...
      if !resposta_palavrachave = 's' then
	    	...
	    	let lista_token = ref (Str.split separa_token (String.sub (List.nth !lista_final i)
			    ...
			    let token_total = ref [] in
					if (List.nth !lista_token 1) = "meno" then
						begin
						lista_token := replace !lista_token 1 "<";					
						end;
					if (List.nth !lista_token 1) = "maio" then
						begin
						lista_token := replace !lista_token 1 ">";					
						end;
	...
	if not (exists !lista_final "apl") then
		begin
		let tamanho_lista = (List.length !lista_final) in
		for i = 0 to (tamanho_lista - 1) do
	  let tamanho_elemento = String.length (List.nth !lista_final i) in
	  let lista_token = ref (Str.split separa_token (String.sub (List.nth !lista_final i)
				 1 (tamanho_elemento - 2))) in					
	      if contains (List.nth !lista_final i) "var" then
			    begin
			    let var_indice = ref 0 in
				  pega_indice (List.nth !lista_token 1) !listaVariaveis var_indice;
			    let valor = (List.nth !listaValores !var_indice) in
					Printf.fprintf oc "%s " valor;
				  end
				else
					begin
					Printf.fprintf oc "%s " (List.nth !lista_token 1);
					end;
				done;
			end;;			
